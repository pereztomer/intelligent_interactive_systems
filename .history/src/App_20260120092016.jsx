  ...
  const convertMarkdownToHTML = (text) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmed.match(/^[\*\-]\s+/)) {
        ...
        // Remove the bullet marker and convert **text** to <strong>text</strong>
        const content = trimmed.replace(/^[\*\-]\s+/, '')
        const boldContent = content.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
        htmlLines.push(`<li>${boldContent}</li>`)
      } else {
        ...
        if (trimmed && (trimmed === trimmed.toUpperCase() || trimmed.endsWith(':'))) {
          const boldContent = trimmed.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
          htmlLines.push(`<h4 style="margin-top: 1rem; margin-bottom: 0.5rem; font-weight: 600;">${boldContent}</h4>`)
        } else if (trimmed) {
          // Regular line - convert **text** to <strong>text</strong>
          const boldContent = trimmed.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
          htmlLines.push(`<p style="margin: 0.5rem 0;">${boldContent}</p>`)
        } else {
          // Empty line
          htmlLines.push('<br />')
        }
