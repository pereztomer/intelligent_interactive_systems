...
function App() {
  ...
  const convertMarkdownToHTML = (text) => {
    ...
    // First, convert **text** to <strong>text</strong> throughout
    ...
    lines.forEach((line) => {
      ...
      if (trimmed.match(/^[\*\-]\s+/)) {
        if (!inList) {
          htmlLines.push('<ul style="margin: 0.5rem 0; padding-left: 1.5rem;">')
          inList = true
        }
        // Remove the bullet marker
        const content = trimmed.replace(/^[\*\-]\s+/, '')
        htmlLines.push(`<li style="margin: 0.25rem 0;">${content}</li>`)
      } else {
        ...
        if (trimmed && (trimmed === trimmed.toUpperCase() && trimmed.length < 50 && trimmed.endsWith(':'))) {
          htmlLines.push(`<h4 style="margin-top: 1.5rem; margin-bottom: 0.75rem; font-weight: 600; color: #2d3748;">${trimmed}</h4>`)
        } else if (trimmed) {
          // Regular line
          htmlLines.push(`<p style="margin: 0.5rem 0; line-height: 1.6;">${trimmed}</p>`)
        } else {
                ...
                <div className="attempt-feedback-box">
                  ...
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ 
                      __html: convertMarkdownToHTML(currentAttempt.geminiFeedback) 
                    }}
                  />
                </div>
              )}
