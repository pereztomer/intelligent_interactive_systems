  ...
  useEffect(() => {
    const loadVideo = async () => {
      try {
        ...
        // Support both old format (base64) and new format (file paths)
        if (recording.videoData) {
          // Old format: use base64 data directly
          setVideoUrl(recording.videoData)
          return
        }
        
        // New format: load from file system
        if (!recording.folderName || !recording.videoFileName) {
          setLoadingError('Recording file information is missing.')
          return
        }
