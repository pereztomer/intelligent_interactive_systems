        ...
        <div className="presentation-viewer">
          <div className="controls-top">
            <div className="navigation-controls">
              <button onClick={goToPrevPage} disabled={pageNumber <= 1 || currentAttempt}>
                ← Previous
              </button>
              <span className="page-counter">
                Page {pageNumber} of {numPages || '...'}
              </span>
              <button onClick={goToNextPage} disabled={pageNumber >= numPages || currentAttempt}>
                Next →
              </button>
            </div>
          ...
          {numPages && numPages > 1 && (
            <div className="page-thumbnails">
              {Array.from({ length: numPages }, (_, idx) => (
                <div
                  key={idx + 1}
                  className={`thumbnail ${idx + 1 === pageNumber ? 'active' : ''} ${currentAttempt ? 'disabled' : ''}`}
                  onClick={() => {
                  ...
                  style={currentAttempt ? { cursor: 'not-allowed', opacity: 0.5 } : {}}
                >
                  {idx + 1}
                </div>
              ))}
