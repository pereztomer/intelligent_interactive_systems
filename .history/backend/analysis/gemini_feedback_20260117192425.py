...
def _generate_with_fallback(prompt):
    ...
    Tries gemini-1.5-flash first (free tier), then gemini-1.5-pro if quota exceeded.
    ...
    """
    # Try gemini-1.5-flash first (available on free tier)
    try:
        ...
        if 'quota' in error_str or '429' in error_str or 'resourceexhausted' in error_str:
            print(f"⚠️  Free tier quota exceeded for gemini-1.5-flash, trying gemini-1.5-pro...")
            try:
